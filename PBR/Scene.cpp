...
void Scene::CreateCornellBoxScene() {
	...
	rootObject = new SceneObject("root");
	SceneObject* ww = new SceneObject("White Walls", Transform(), 1);
	ww->SetMesh(Mesh(back));
	SceneObject* gw = new SceneObject("Green Wall", Transform(), 0);
	gw->SetMesh(Mesh(left));
	SceneObject* rw = new SceneObject("Red Wall", Transform(), 2);
	rw->SetMesh(Mesh(right));
	SceneObject* ls = new SceneObject("Light Source", Transform(), 3);
	...
	SceneObject* cube = new SceneObject("Cube", Transform(glm::vec3(130, 0, 65) + box1Size / 2.0f, glm::vec3(0, glm::radians(-18.0), 0)), 1);
	cube->SetMesh(Mesh::CreateBox(box1Size));
	rootObject->AddChild(cube);

	SceneObject* box = new SceneObject("Cube", Transform(glm::vec3(265, 0, 295) + box2Size / 2.0f, glm::vec3(0, glm::radians(15.0), 0)), 1);
...
bool Scene::CheckCollision(Ray ray, float tMin, float tMax, CollisionInfo& outCollision) const {
	...
	return hitAnything;
	//return spaceTree->CheckCollision(ray, tMin, tMax);
}
